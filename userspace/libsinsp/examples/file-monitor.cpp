...
void print_event_details(sinsp_evt* evt, sinsp& inspector)
{
	...
	// Our custom "fim_type"
	std::string fim_type       = determine_fim_type(evt, inspector);
...
int main(int argc, char** argv) {
    if (argc < 3) {
        cerr << "Usage: " << argv[0] << " <bpf_probe_path> <directory_to_monitor>" << endl;
        return 1;
    }

    string bpf_path = argv[1];
    monitored_directory = argv[2];
    ...
    try {
        ...
        inspector.open_bpf(bpf_path.c_str(), DEFAULT_DRIVER_BUFFER_BYTES_DIM, events_sc_codes);
